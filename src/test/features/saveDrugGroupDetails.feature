Feature:Retrieve and validate the save drug group details
  ...
  Background:
    Given User executes the delete existing DrugGroup query "deleteDrugGroupByName" and List name "TestDrugGroupData" to delete the record from the database
    Given User create the Create new group details data
      | mfrId    | drugGroupName      | drugGroupType |
      | ASTRA001 | TestDrugGroupData | Rebateable    |
