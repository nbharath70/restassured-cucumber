...
Feature:Retrieve and validate the LOBs for a  Contract
  ...
  Scenario: get LOBS for a valid contract and verify the correct status code is returned
    ...
    When User hits "getLOBofaContractRowkeyResource" API
  ...
  Scenario: get LOBS for an Invalid contract and verify the error msg
    Given User executes "sqlToPickInvalidContract" and gets value for column "columnNameToGetContractRowkey"
    When User hits "getLOBofaContractRowkeyResource" API
  ...
  Scenario: get LOBS for a contract with type mismatch and verify the error msg
    Given User Hits "getLOBofaContractRowkeyResource" with rowKey "typeMissMactchedRoeKey" and Get API request
  ...
  Scenario: get LOBS for a contract with Is_current_Flag 0 and verify the error msg
    Given User executes "sqlToPickAContractWithIsCurrentFlagZero" and gets value for column "columnNameToGetContractRowkey"
    When User hits "getLOBofaContractRowkeyResource" API
